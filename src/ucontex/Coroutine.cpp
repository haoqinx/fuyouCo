...
int CoScheduler::createCoroutine(coroutineFunc func, void* args){
    std::shared_ptr<Coroutine> newCo = std::shared_ptr<Coroutine>(new Coroutine(std::move(func), args, this));
    int id;
...
void CoScheduler::resume(int id){
    ...
    std::shared_ptr<Coroutine> co = coroutines_[id];
    if(co) return;
...
void CoScheduler::mainfunc(uint32_t low32, uint32_t h32){
    uintptr_t ptr = (uintptr_t)low32 | ((uintptr_t)h32 << 32);
    CoScheduler* sche = (CoScheduler*)ptr;
    int id = sche -> runningCo_;
    std::shared_ptr<Coroutine> co = sche -> coroutines_[id];
    co -> dofunc();
    co.reset();
    -- sche -> numCoroutines_;
    sche -> runningCo_ = -1;
